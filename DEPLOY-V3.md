...
function renderServiceButtons(){
 $("#serviceButtons").innerHTML=db.services.filter(s=>s.active).map(s=>`<button class="service-btn" data-service="${s.id}">${s.imageData?`<img class="service-btn-photo" src="${s.imageData}" alt="${esc(s.name)}">`:""}<b>${esc(s.name)}</b><span>${s.type==="kg"?"Per kilo":s.type==="piece"?"Per piece":"Flat rate"}</span><strong>${money(s.rate)}</strong></button>`).join("");
 $$("[data-service]").forEach(b=>b.onclick=()=>openServiceItem(b.dataset.service))
}
...
let pendingServicePhotoData="";

function resizeImageFile(file,maxWidth=900,maxHeight=700,quality=.78){
 return new Promise((resolve,reject)=>{
   if(!file)return resolve("");
   if(file.size>6*1024*1024)return reject(new Error("Photo must be below 6MB"));
...
function showServicePhotoPreview(data){
 const p=$("#sPhotoPreview");
 if(!p)return;
 if(data){p.src=data;p.classList.remove("hidden")}
 else{p.removeAttribute("src");p.classList.add("hidden")}
}

async function publishOnlineCatalog(silent=false){
 const key=String(db.settings.bookingAdminKey||"").trim();
 if(!key){if(!silent)toast("Add ADMIN API key in Online Booking settings first");return false}
 const services=db.services.map(s=>({
   id:s.id,name:s.name,description:s.description||"",
   type:s.type,rate:Number(s.rate||0),minimum:Number(s.minimum||0),
   active:!!s.active,show_online:s.showOnline!==false,
 ...
 try{
   ...
   if(!res.ok)throw new Error(data.error||"Unable to publish online catalog");
   const status=$("#catalogPublishStatus");
   if(status)status.textContent=`Published ${data.count||services.length} services · ${new Intl.DateTimeFormat("en-PH",{timeZone:"Asia/Manila",dateStyle:"medium",timeStyle:"medium"}).format(new Date())}`;
   if(!silent)toast("Online catalog published");
   return true;
 }catch(error){
   console.error("Catalog publish error:",error);
   const status=$("#catalogPublishStatus");if(status)status.textContent="Publish failed: "+error.message;
...
function renderServices(){
 ...
 $("#servicesTable").innerHTML=db.services.filter(s=>JSON.stringify(s).toLowerCase().includes(q)).map(s=>`<tr>
 <td>${s.imageData?`<img class="service-thumb" src="${s.imageData}" alt="${esc(s.name)}">`:`<div class="service-thumb-placeholder">🧺</div>`}</td>
 <td><b>${esc(s.name)}</b><br><small>${esc(s.description||"")}</small></td>
 <td>${s.type==="kg"?"Per kilo":s.type==="piece"?"Per piece":"Flat rate"}</td>
 <td>${money(s.rate)}</td><td>${s.minimum}</td>
 <td>${s.showOnline!==false?'<span class="online-badge">ONLINE</span>':'<span class="offline-badge">HIDDEN</span>'}</td>
 <td>${s.active?"Yes":"No"}</td><td><button data-edit-service="${s.id}">Edit</button></td></tr>`).join("");
 $$("[data-edit-service]").forEach(b=>b.onclick=()=>openService(db.services.find(s=>s.id===b.dataset.editService)))
}
function openService(s=null){
 $("#serviceDialogTitle").textContent=s?"Edit Service":"Add Service";
 $("#serviceId").value=s?.id||"";$("#sName").value=s?.name||"";
 $("#sDescription").value=s?.description||"";
 $("#sType").value=s?.type||"kg";$("#sRate").value=s?.rate??"";
 $("#sMinimum").value=s?.minimum??0;$("#sOnline").checked=s?.showOnline??true;
 $("#sActive").checked=s?.active??true;
 pendingServicePhotoData=s?.imageData||"";
 showServicePhotoPreview(pendingServicePhotoData);
 $("#sPhotoInput").value="";
 $("#serviceDialog").showModal()
}
async function saveService(e){
 e.preventDefault();
 const id=$("#serviceId").value||uid("s");
 const existing=db.services.find(x=>x.id===id);
 const s={
   id,name:$("#sName").value.trim(),description:$("#sDescription").value.trim(),
   type:$("#sType").value,rate:Number($("#sRate").value),
   minimum:Number($("#sMinimum").value||0),active:$("#sActive").checked,
   showOnline:$("#sOnline").checked,imageData:pendingServicePhotoData||existing?.imageData||""
 };
 const ix=db.services.findIndex(x=>x.id===id);
 if(ix>=0)db.services[ix]=s;else db.services.push(s);
 save();$("#serviceDialog").close();renderAll();toast("Service saved");
 publishOnlineCatalog(true);
}
...
function bind(){
 ...
 $("#addServiceBtn").onclick=()=>openService();$("#sPhotoInput").onchange=async e=>{try{pendingServicePhotoData=await resizeImageFile(e.target.files[0]);showServicePhotoPreview(pendingServicePhotoData);toast("Photo ready")}catch(error){toast(error.message)}};$("#removeServicePhotoBtn").onclick=()=>{pendingServicePhotoData="";showServicePhotoPreview("")};$("#publishCatalogBtn").onclick=()=>publishOnlineCatalog(false);$("#saveServiceDialogBtn").onclick=saveService;$("#servicesSearch").oninput=renderServices;$("#addServiceItemBtn").onclick=addServiceItem;
