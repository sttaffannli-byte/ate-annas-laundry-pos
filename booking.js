...
function load(){
 try{
   ...
   const merged={...structuredClone(defaults),...x,settings:{...defaults.settings,...(x.settings||{})}};
   if(!Array.isArray(merged.customers)||!merged.customers.length)merged.customers=structuredClone(defaults.customers);
   if(!Array.isArray(merged.services)||!merged.services.length)merged.services=structuredClone(defaults.services);
...
function renderAll(){
 const modules=[
   ...
   ["Online Booking",renderBookingAdmin],
   ["Customers",renderCustomers],
   ["Service Buttons",renderServiceButtons],
   ["Order Items",renderOrderItems],
 ...
 modules.forEach(([name,fn])=>{
   try{fn()}
   catch(error){console.error(name+" render error:",error);errors.push(name)}
 });
 if(errors.length)toast("Loaded with repair mode: "+errors.join(", "));
}
...
function startStablePOS(){
 ...
 if("serviceWorker"in navigator){
   navigator.serviceWorker.register("/sw.js?v=5.2.1-render-fix",{updateViaCache:"none"})
     .then(reg=>reg.update())
